...
namespace BTL.View1
{
    public partial class frmNhanVien : Form
    {
        ...
        //gán dữ liệu
        void ganDuLieu(NhanVienObj nvObj,String action)
        {
            nvObj.Idnv = int.Parse(txtma.Text);
            nvObj.Idpb = int.Parse((cbxDepartment.SelectedValue).ToString());
            nvObj.Idchucvu = int.Parse((cbxPosition.SelectedValue).ToString());
            nvObj.Ten = txtten.Text.Trim();
            if (cbgioitinh.Text.Trim().Equals("Nam"))
            {
                nvObj.Gioitinh = true;
            }else if (cbgioitinh.Text.Trim().Equals("Nữ"))
            {
                nvObj.Gioitinh = false;
            }
            nvObj.Cmnd = txtcmnd.Text.Trim();
            nvObj.Que = txtquequan.Text.Trim();
            nvObj.Hsl = float.Parse(txthsl.Text.Trim());
            nvObj.Sdt = txtsdt.Text.Trim();
            nvObj.Trinhdohocvan = txttrinhdo.Text.Trim();
            nvObj.Email = txtemail.Text.Trim();
            nvObj.Hanhopdong = dthanhopdong.Value;
            if (action.Equals("Add"))
            {
                nvObj.CreatedDate = DateTime.Now;
            }
            nvObj.Ngaykyhopdong = dtngayky.Value;
            nvObj.Ngaysinh = dtngaysinh.Value;
            if (action.Equals("Edit"))
            {
                nvObj.UpdateDate = DateTime.Now;
            }
